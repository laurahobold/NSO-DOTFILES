local wibox = require("wibox")
local awful = require("awful")
local gears = require("gears")
local beautiful = require("beautiful")
local dpi = beautiful.xresources.apply_dpi

--Custom Modules
local color = require("popups.color")
local user = require("popups.user_profile")

local icon_path = os.getenv("HOME") .. '/.config/awesome/popups/dashboard/home/widgets/web_icons/'

local tooltip = wibox.widget {
    image = ".config/awesome/popups/control_center/assets/tooltip.png",
    resize = true,
    widget = wibox.widget.imagebox,
}

local Separator = wibox.widget.textbox("    ")
Separator.forced_height = 10
Separator.forced_width = 10

local create_button = function(icon, command)
    local image = wibox.widget {
        image = icon,
        widget = wibox.widget.imagebox,
        resize = true,
        forced_height = dpi(40),
        forced_width = dpi(40),
    }

    local button = wibox.widget {
        top = dpi(-180),
        {
            image,
            widget = wibox.container.place,
        },
        widget = wibox.container.margin,
    }
    return button
end

local reddit = create_button(icon_path .. 'reddit.png', 'https://www.reddit.com/')
local github = create_button(icon_path .. 'git.png', 'https://github.com/')
local youtube = create_button(icon_path .. 'yt.png', 'https://www.youtube.com/')

---------------------------
--Main widget--------------
---------------------------

local web_launch = wibox.widget {
    tooltip,
    layout = wibox.layout.stack,
    {
        {
            reddit,
            Separator,
            github,
            Separator,
            youtube,
            layout = wibox.layout.fixed.horizontal,
        },
        widget = wibox.container.margin,
        left = dpi(35),
    },
    widget = wibox.container.margin,
    bottom = dpi(-210),
    forced_width = dpi(210),
    forced_height = dpi(0),
}

-------------------------
--Sys_info---------------
-------------------------

---Ram-----------------
local text1 = '<span color="' ..
        color.blueish_white .. '" font="DinkieBitmap 15">' .. '  :   ' .. '1.58 GiB' .. '</span>'
local icon1 = '<span color="' .. color.red .. '" font="Ubuntu Nerd Font bold 15">' .. '' .. '</span>'

local ram_usage = wibox.widget {
    markup = icon1 .. text1,
    font = "DinkieBitmap 15",
    widget = wibox.widget.textbox,
    fg = color.white,
    forced_height = dpi(30),
}

local ram_text = wibox.widget {
    ram_usage,
    widget = wibox.widget.textbox,
    widget = wibox.container.margin,
    bottom = dpi(-70),
    right = dpi(9),
    left = dpi(25),
    forced_height = dpi(30)
}

-- Fetch ram_usage info
awful.spawn.easy_async_with_shell([[ free -m | awk 'NR==2 {print $3}'
]], function(out)
    local str = string.gsub(out, "%s+", "")
    local val = tonumber(str) / 1000
    ram_usage.markup = icon1 .. '<span color="' ..
            color.blueish_white .. '" font="Ubuntu Nerd Font bold 17">' .. '  :   ' .. val .. ' GiB' .. '</span>'
end)

--Update ram_usage every time
local update_ram = function()
    awful.spawn.easy_async_with_shell([[ free -m | awk 'NR==2 {print $3}'
]], function(out)
        local str = string.gsub(out, "%s+", "")
        local val = tonumber(str) / 1000
        ram_usage.markup = icon1 .. '<span color="' ..
                color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. val .. ' GiB' .. '</span>'
    end)
end

local update_ram_timer = gears.timer({
    timeout = 30,
    call_now = true,
    autostart = true,
    callback = update_ram
})

---Cpu----------------------
local text2 = '<span color="' ..
        color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. '1%' .. '</span>'
local icon2 = '<span color="' .. color.red .. '" font="DinkieBitmap 15">' .. '' .. '</span>'

local cpu_usage = wibox.widget {
    -- text = user.name,
    markup = icon2 .. text2,
    font = "DinkieBitmap 14",
    widget = wibox.widget.textbox,
    fg = color.white,
    -- forced_width = dpi(300),
}

local cpu_text = wibox.widget {
    cpu_usage,
    font = "DinkieBitmap 14",
    widget = wibox.widget.textbox,
    widget = wibox.container.margin,
    bottom = dpi(-60),
    right = dpi(9),
    left = dpi(25),
    forced_height = dpi(30)
},

-- Fetch cpu_usage info
awful.spawn.easy_async_with_shell([[ echo ""$[100-$(vmstat 1 2|tail -1|awk '{print $15}')]"%"
]], function(out)
    local str = string.gsub(out, "%s+", "")

    cpu_usage.markup = icon2 .. '<span color="' ..
            color.blueish_white .. '" font="DinkieBitmap 17">' .. '   :   ' .. str .. '</span>'
end)

--Update cpu_usage every time
local update_cpu = function()
    awful.spawn.easy_async_with_shell([[ echo ""$[100-$(vmstat 1 2|tail -1|awk '{print $15}')]"%"
]], function(out)
        local str = string.gsub(out, "%s+", "")

        cpu_usage.markup = icon2 .. '<span color="' ..
                color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. str .. '</span>'
    end)
end

local update_cpu_timer = gears.timer({
    timeout = 10,
    call_now = true,
    autostart = true,
    callback = update_cpu
})

---Storage----------------------
local text3 = '<span color="' ..
        color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. '69/334 GiB' .. '</span>'
local icon3 = '<span color="' .. color.red .. '" font="DinkieBitmap 15">' .. '󰨣' .. '</span>'

local storage_usage = wibox.widget {
    markup = icon3 .. text3,
    font = "Ubuntu Nerd Font bold 17",
    widget = wibox.widget.textbox,
    fg = color.white,
}

local storage_text = wibox.widget {
    storage_usage,
    widget = wibox.widget.textbox,
    widget = wibox.container.margin,
    bottom = dpi(-50),
    right = dpi(9),
    left = dpi(25),
    forced_height = dpi(30)
}

-- Fetch storage_usage info
awful.spawn.easy_async_with_shell([[ df -h --total | tail -n 1 | awk '{print $3 "/" $4}'
]], function(out)
    local str = string.gsub(out, "%s+", "")

    storage_usage.markup = icon3 .. '<span color="' ..
            color.blueish_white .. '" font="DinkieBitmap 17">' .. '   :   ' .. str .. '</span>'
end)

--Update storage_usage every time
local update_storage = function()
    awful.spawn.easy_async_with_shell([[df -h --total | tail -n 1 | awk '{print $3 "/" $4}'
]], function(out)
        local str = string.gsub(out, "%s+", "")

        storage_usage.markup = icon3 .. '<span color="' ..
                color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. str .. '</span>'
    end)
end

local update_storage_timer = gears.timer({
    timeout = 7200,
    call_now = true,
    autostart = true,
    callback = update_storage
})

---Package----------------------
local text4 = '<span color="' ..
        color.blueish_white .. '" font="DinkieBitmap 15">' .. '   :   ' .. '2634' .. '</span>'
local icon4 = '<span color="' .. color.red .. '" font="DinkieBitmap 15">' .. '󰏖' .. '</span>'

local bg = wibox.widget {
    image = ".config/awesome/popups/control_center/assets/348.png",
    resize = true,
    widget = wibox.widget.imagebox,
}

local packages_text = wibox.widget {
    {
        markup = icon4 .. text4,
        font = "DinkieBitmap 14",
        widget = wibox.widget.textbox,
        fg = color.white,
    },
    widget = wibox.container.margin,
    bottom = dpi(-40),
    right = dpi(9),
    left = dpi(25),
    forced_height = dpi(30),
}

local Sys_info = wibox.widget {
    {

        {
            bg,
            layout = wibox.layout.stack,
            widget = wibox.container.margin,
            {
                ram_text,
                cpu_text,
                storage_text,
                packages_text,
                widget = wibox.container.margin,
                layout = wibox.layout.fixed.vertical,
            },
        },
        widget = wibox.container.background,
    },
    widget = wibox.container.margin,
}

local main = wibox.widget {
    web_launch,
    Separator,
    Sys_info,
    Separator,
    layout = wibox.layout.fixed.horizontal,
}

return main

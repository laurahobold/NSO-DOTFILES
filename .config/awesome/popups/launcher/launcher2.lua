      ...
      {
        {
          {
            {
              image = os.getenv("HOME") .. '/.config/awesome/popups/launcher/44.png',
      ...
      {
        {
...

          {
            -- text = entry.name,
            markup = '<span color="' ..
                color.blueish_white .. '" font="DinkieBitmap 13">' .. entry.name .. '</span>',
...
local function open()
  ...
  awful.prompt.run {
    -- prompt = "Search: ",
    prompt = '<span color="' ..
        color.blueish_white .. '" font="DinkieBitmap 16">' .. '  : ' .. '</span>',
    textbox = prompt,
